from functions import extract_text_from_all_pdf, extract_text_from_pdf, text_to_chunk, get_vectorstore, get_conv, \
    load_model, load_modeltext, similarity_check
...
def main_section_one(uploaded_file, placeholder, section_options, text):
            ...
            for i in range(len(st.session_state.summary)):
                ...
                with col1:
                    if (st.session_state.file_name[i].split(".")[-1] == 'pdf'):
                        ...
                        displayPDF(filepath)
                    elif (st.session_state.file_name[i].split(".")[-1] == 'jpg' or
                          st.session_state.file_name[i].split(".")[-1] == 'jpeg' or
                          st.session_state.file_name[i].split(".")[-1] == 'png'):
                        st.info("Uploaded File " + str(st.session_state.file_name[i]))
                        st.image(filepath)
                with col2:
    ...
    if (uploaded_file or text):
        option = st.selectbox("Choose the language", ("English", "Spanish"), placeholder="Select Language")
        ...
        if st.button("Summarize"):
                ...
                for i in uploaded_file:
                    ...
                    with col1:
                        if (str(i.name).split(".")[-1] == 'pdf'):
                            ...
                            displayPDF(filepath)
                        elif (str(i.name).split(".")[-1] == 'jpg' or str(i.name).split(".")[-1] == 'jpeg' or
                              str(i.name).split(".")[-1] == 'png'):
                            st.info("Uploaded File " + str(i.name))
                            st.image(i)
                    with col2:
            ...
            if text:
                ...
                with col2:
                    ...
                    if (summary):
                        if (option == 'Spanish'):
                            translation = translate(summary, 'sp')
                            sum.append(translation)
                        else:
                            sum.append(summary)
                        a.empty()
